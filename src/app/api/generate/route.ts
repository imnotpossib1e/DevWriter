...
4. **코드 블록 바로 앞 colon(:)콜론 기호 절대 사용 금지**
  올바른 예시: "설명 내용\n\n\`\`\`tsx\n코드\`\`\`"
  틀린 예시: "설명 내용:\n\n\`\`\`tsx\n코드\`\`\`" **절대 금지**
...
8. 글자수는 코드 예시 포함합니다.
9. 글자수 꼭 지키세요.
10. 섹션별로 제목을 붙이기(**마크다운 형식**)
11. **서론, 본론, 결론 같은 섹션명은 사용하지 않는다**
    ...
    tutorial: 기술/라이브러리 사용법을 처음부터 끝까지 단계별로 설명할 때 사용
      ...
      - content 구조 (*필수)
          - 제목*
              - 주제+핵심 이득이 들어나는 제목
          - 인트로
              - 문제 제기(왜 알아야하는지) + 글에서 다룰 내용 요약
              - 2~4문단
          - 사전 준비
              - 선행 지식, 필요 환경(React 버전, 언어 등)
              - 불릿 리스트 형식
          - steps[]* (최소 2개 이상)
              - 설명: 튜토리얼의 각 단계
              - step 필드
    ...
    TIL: 오늘늘 새로 알게 된 지식이나 인사이트 짧고 솔직하게 기록
      ...
      - content 구조 (*필수)
          ...
          - 핵심 내용*
              ...
              - 불릿 리스트나 짧은 문단
          - 코드 예시*
              - 관련 코드 예시를 주석으로 설명과 함께 작성
          - 느낀 점*
              - 느낀 점, 앞으로 어떻게 적용할지, 주의할 점
              - 1~2문단`,
    troubleShooting: `
    Trouble Shooting: 에러/버그가 발생한 상황과 해결 과정을 기록
      ...
      - content 구조 (*필수)
          ...
          - 환경
              ...
              - 키-값 리스트 형식
          - 문제 상황*
              - 어떤 증상이 발생했는지, 기대 결과 vs 실제 결과, 콘솔/로그에 찍힌 에러 메시지
          - 원인 분석*
              - 원인을 찾기까지의 추론 과정, 잘못 짚은 가설도 같이
          - 해결 방법*
              - 실제로 적용한 해결 단계
              - step 필드
                  - 제목
